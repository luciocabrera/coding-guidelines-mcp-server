...
function needsClarification(query?: string): boolean {
  ...
  const hasVerb = /\b(add|create|generate|make|improve|enhance|fix|debug|review|summarize|validate|search|bootstrap|style|design|restyle|refactor|optimize|test)\b/i.test(
    text
  );

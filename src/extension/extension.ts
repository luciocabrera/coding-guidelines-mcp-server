...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const coding = vscode.chat.createChatParticipant('coding-guidelines.coding', handleChatRequest);

  coding.iconPath = new vscode.ThemeIcon('gear');

  const standards = vscode.chat.createChatParticipant(
    'coding-guidelines.standards',
    handleChatRequest,
  );

  standards.iconPath = new vscode.ThemeIcon('shield');

  context.subscriptions.push(agent);
  context.subscriptions.push(guardian);
  context.subscriptions.push(coding);
  context.subscriptions.push(standards);
...
function getHelpMessage(): string {
  return [
    ...
    '- @guardian [query] — Standards-focused persona',
    '- @coding [query] — Alias for @guidelines',
    '- @standards [query] — Alias for @guardian',
...
async function ensureScriptsForCommands(
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    stream.markdown(`⚠️ Unable to ensure scripts: ${errorMessage}\n`);
  }
}
    ...
    try {
      ...
      if (!attemptedRetry && isMissingScript) {
        ...
        try {
          ...
          stream.markdown(`⚠️ Command failed again: ${cmd} - ${retryErrorMessage}\n`);
          break;
        }
      }

      const errorMessage = error instanceof Error ? error.message : String(error);
      stream.markdown(`⚠️ Command failed: ${cmd} - ${errorMessage}\n`);
      break;
    }
  }
}

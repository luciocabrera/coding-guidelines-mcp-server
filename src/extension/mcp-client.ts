...
export class MCPClient {
  ...
  async generateCode(payload: {
    ...
    includeRef?: boolean;
  }): Promise<{ text: string; files?: Array<{ path: string; content: string }>; commands?: string[] }> {
    const result = (await this.sendRequest("tools/call", {
      name: "generate_code",
      arguments: payload,
    })) as { content: Array<{ text: string }>; files?: Array<{ path: string; content: string }>; commands?: string[] };
